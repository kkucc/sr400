    ...
    def read_data(self):
        ...
        while self.reading:
            try:
                ...
                self.sr400.write(f"CP2,{tset*10**7}\n") # Set preset
...
root.mainloop()# --- START OF FILE modified_1522reader.py (External Library) ---
import tkinter as tk
import threading
import time
import csv
...
class App:
    ...
    def start_reading(self):
        ...
        if self.file_available and not self.reading:
            self.reading = True
            self.start_button.config(state=tk.DISABLED)
            self.stop_button.config(state=tk.NORMAL)

            if not self.data_thread or not self.data_thread.is_alive():
              self.data_thread = threading.Thread(target=self.read_data_from_file)
              self.data_thread.daemon = True
              self.data_thread.start()
    ...
    def stop_reading(self):
        ...
        if self.reading:
            ...
            if self.data_source and not isinstance(self.data_source, str):
                ...
                self.data_source.stop_acquisition()  # Assuming a method to stop
        self.reset_data()
...
# --- END OF FILE modified_1522reader.py ---