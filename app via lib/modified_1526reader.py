...
class App:
    def __init__(self, root, data_source=None):
        ...
        self.MAX_DATA_POINTS = 10000 # увеличил для отображения большего кол-ва экспериментов
        ...
        self.counts = []  # Массив для хранения номеров итераций
        self.global_iteration = 0 # номер итерации с начала запуска программы
    ...
    def create_row_3(self):
        ...
        self.x_label = tk.Label(self.x_frame, text="Sum:", font=self.FONT_STYLE, fg=self.VALUE_FG,
                                bg=self.VALUE_BG, width=self.LABEL_WIDTH)
    ...
    def update_plot(self):
        ...
        if len(self.counts) > self.MAX_DATA_POINTS:
            # self.counts = self.counts[-self.MAX_DATA_POINTS:]
            ...
            # Если нужно оставить старые и не удалять
            del self.counts[:self.MAX_DATA_POINTS]
            del self.a_values[:self.MAX_DATA_POINTS]
            del self.b_values[:self.MAX_DATA_POINTS]
            del self.x_values[:self.MAX_DATA_POINTS]

        self.ax.clear()
        if self.counts:
            self.ax.plot(self.counts, self.a_values, label='A', linestyle='-', color='blue')
            self.ax.plot(self.counts, self.b_values, label='B', linestyle='-', color='red')
            self.ax.plot(self.counts, self.x_values, label='Sum', linestyle='-', color='green')
    ...
    def process_data_queue(self):
        ...
        while not self.data_queue.empty():
            ...
            self.counts.append(self.global_iteration)
