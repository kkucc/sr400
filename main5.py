...
import time
import numpy as np
import modified_1526reader as reader  # Импортируем измененный файл
...
class MainApp:
    ...
    def connect_to_sr400(self):
        ...
        try:
            ...
            for resource in resources:
                if 'ASRL5' in resource: #изменил
                    try:
                        temp_inst = rm.open_resource(resource, timeout=5000) # Добавляем таймаут
                        temp_inst.close()
    ...
    def create_tset_control(self):
        ...
        num_periods_frame = tk.Frame(self.reader_app.row3_frame, bg=self.reader_app.VALUE_BG, bd=2, relief=tk.GROOVE)
    ...
    def update_tset(self, event):
        ...
        try:
            ...
            if 0.000001 <= new_tset <= 1: #изменил
